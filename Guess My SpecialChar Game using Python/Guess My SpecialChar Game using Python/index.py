...
def register():
    ...
    register_title=tk.Label(register_window, text="Register", font=('Arial', 40))
...
def login():
    ...
    login_title=tk.Label(login_window, text="Login", font=('Arial', 40))
...
def account_screen():
    ...
    account_window.title("SpecialChar Game")
...
def single_player_settings():
    ...
    header=tk.Label(single_player_window, text="Single Player Mode", font=('Impact', 80))
    header.pack(fill="x", pady=10)
    
    attempts_label=tk.Label(single_player_window, text="How many attempts do you want? (4-15) :", font=('Arial', 25))
    attempts_label.pack(anchor='center', pady=5)
    attempts_entry=tk.Entry(single_player_window,font=('Arial', 22))
    attempts_entry.pack(anchor='center', pady=5)
    
    chr_choice_label=tk.Label(single_player_window, text="How many characters do you want? (4-10) :", font=('Arial', 25))
    chr_choice_label.pack(anchor='center', pady=5)
    chr_choice_entry=tk.Entry(single_player_window, font=('Arial', 22))
...
def single_player_game():
    ...
    sp_gamewindow.title("SpecialChar Game")
  
    
    header=tk.Label(sp_gamewindow, text="SpecialChar Game", font=("Impact", 80, "bold"))
    header.pack(fill="x")
    
    rp_frame=tk.Frame(sp_gamewindow)
    rp_frame.pack(anchor="center")
    
    reveal_frame=tk.Frame(rp_frame)
    ...
    chr_frame=tk.Frame(sp_gamewindow)
    chr_frame.pack(anchor="center", pady=20)
    chr_button_dict={}
    
    for x in range(len(chrs)):
        chr_button_dict[chrs[x]]=tk.Button(chr_frame, text=chrs[x], font=('Arial', 19, 'bold'),command=lambda j=chrs[x]: character_clicked(j))
    ...
    play_again=tk.Button(sp_gamewindow, text="Play Again", font=("Arial", 27, "bold"),
                         command=lambda: play_single_player_again())
...
def double_player_settings():
    ...
    header=tk.Label(double_player_window, text="Double Player Mode", font=('Impact', 70))
    header.pack(fill="x", pady=5)
    
    player1_name_label=tk.Label(double_player_window, text="Name of Player 1: ", font=('Arial', 25))
    player1_name_label.pack(anchor='center', pady=3)
    player1_name_entry=tk.Entry(double_player_window, font=('Arial', 20))
    player1_name_entry.pack(anchor='center', pady=3)

    player2_name_label=tk.Label(double_player_window, text="Name of Player 2: ", font=('Arial', 25))
    player2_name_label.pack(anchor='center', pady=3)    
    player2_name_entry=tk.Entry(double_player_window, font=('Arial', 20))
    player2_name_entry.pack(anchor='center', pady=3)
    
    attempts_label=tk.Label(double_player_window, text="How many attempts do you want? (4-15): ", font=('Arial', 25))
    attempts_label.pack(anchor='center', pady=3)
    attempts_entry=tk.Entry(double_player_window, font=('Arial', 20))
    attempts_entry.pack(anchor='center', pady=3)
    
    chr_choice_label=tk.Label(double_player_window, text="How many characters do you want? (4-10): ", font=('Arial', 25))
    chr_choice_label.pack(anchor='center', pady=3)
    chr_choice_entry=tk.Entry(double_player_window, font=('Arial', 20))
    chr_choice_entry.pack(anchor='center', pady=3)
    
    note_label=tk.Label(double_player_window, text="Note: Number of attempts must be greater than number of characters.", font=('Arial', 13))
    note_label.pack(pady=3)
    
    ok_button=tk.Button(double_player_window, text="OK", font=('Arial', 20), command=lambda: ok_button_double_player_choice())
    ok_button.pack(anchor='center', pady=3)
    
    go_back=tk.Button(double_player_window, text="Go Back To Main Menu", font=("Arial", 18, "bold"), command=lambda: goback_main_menu_from_dps())
...
def double_player_game():
    ...
    header=tk.Label(dp_gamewindow, text="SpecialChar Game", font=("Impact", 75, "bold"))
    header.pack(fill="x")
    
    game1=tk.Frame(dp_gamewindow)
    
    rp_frame1=tk.Frame(dp_gamewindow)
    rp_frame1.pack(anchor="center")
    
    game_player_label1=tk.Label(rp_frame1, text="Player 1: " + gameplayer1, font=("Arial", 20))
    game_player_label1.pack()
    
    reveal_frame1=tk.Frame(rp_frame1)
    ...
    chr_frame1=tk.Frame(dp_gamewindow)
    chr_frame1.pack(anchor="center")
    chr_button_dict1={}
    
    for x in range(len(chrs)):
        chr_button_dict1[chrs[x]]=tk.Button(chr_frame1, text=chrs[x], font=('Arial', 18, 'bold'), command=lambda j=chrs[x]: dp_cc1(j))
    ...
    rp_frame2=tk.Frame(dp_gamewindow)
    rp_frame2.pack(anchor="center")
    
    game_player_label2=tk.Label(rp_frame2, text="Player 2: " + gameplayer2, font=("Arial", 20))
    game_player_label2.pack()
    
    reveal_frame2=tk.Frame(rp_frame2)
    ...
    chr_frame2=tk.Frame(dp_gamewindow)
    chr_frame2.pack(anchor="center")
    chr_button_dict2={}
    
    for x in range(len(chrs)):
        chr_button_dict2[chrs[x]]=tk.Button(chr_frame2, text=chrs[x], font=('Arial', 18, 'bold'), command=lambda j=chrs[x]: dp_cc2(j))
        
    for x in chr_button_dict2:
        chr_button_dict2[x].pack(side="left", padx=4, pady=5)
    
    status_bar1=tk.Frame(dp_gamewindow, borderwidth=4, relief="raised", bg="red")
    status_bar1.pack(side="left", fill="x", expand=1, padx=5)
    
    status1_1=tk.Label(status_bar1, text="Player 1: " + gameplayer1, borderwidth=2, font=('Arial', 25), bg="red")
    status1_1.pack(fill="x", padx=5)
    
    status1_2=tk.Label(status_bar1, text="Score: " + str(score1), borderwidth=2, font=17, bg="red")
    status1_2.pack(fill="x", padx=5)
    
    status1_3=tk.Label(status_bar1, text="Total attempts: " + str(attempts_choice), borderwidth=2, font=17, bg="red")
    ...
    Buttons_Frame=tk.Frame(dp_gamewindow)
    Buttons_Frame.pack(side="left", fill="x", expand=1)
    
    play_again=tk.Button(Buttons_Frame, text="Play Again", font=("Arial", 20, "bold"), command=lambda: play_double_player_again())
    play_again.pack(anchor='center', pady=6)
    
    back_to_dp_settings=tk.Button(Buttons_Frame, text="Back To Settings", font=("Arial", 20, "bold"), command=lambda: backto_dps_and_dpgame_destroy())
    back_to_dp_settings.pack(anchor='center', pady=6)
    
    
    status_bar2=tk.Frame(dp_gamewindow, borderwidth=4, relief="raised", bg="red")
    status_bar2.pack(side="left", fill="x", expand=1, padx=5)
    
    status2_1=tk.Label(status_bar2, text="Player 2: " + gameplayer2, borderwidth=2, font=('Arial', 25), bg="red")
    status2_1.pack(fill="x", padx=5)
    
    status2_2=tk.Label(status_bar2, text="Score: " + str(score2), borderwidth=2, font=17, bg="red")
    status2_2.pack(fill="x", padx=5)
    
    status2_3=tk.Label(status_bar2, text="Total attempts: " + str(attempts_choice), borderwidth=2, font=17, bg="red")
...
def instant_player_settings():
    ...
    header=tk.Label(instant_player_window, text="Instant Player Mode", font=('Impact', 80))
    header.pack(fill="x", pady=5)
    
    attempts_label=tk.Label(instant_player_window, text="How many attempts do you want? (4-15): ", font=('Arial', 25))
    attempts_label.pack(anchor='center', pady=5)
    attempts_entry=tk.Entry(instant_player_window, font=('Arial', 20))
    attempts_entry.pack(anchor='center', pady=5)
    
    chr_choice_label=tk.Label(instant_player_window, text="How many characters do you want? (4-10): ", font=('Arial', 25))
    chr_choice_label.pack(anchor='center', pady=5)
    chr_choice_entry=tk.Entry(instant_player_window, font=('Arial', 20))
    chr_choice_entry.pack(anchor='center', pady=5)
    
    note_label=tk.Label(instant_player_window, text="Note: Number of attempts must be greater than number of characters.", font=('Arial', 15))
    note_label.pack(pady=10)
    
    ok_button=tk.Button(instant_player_window, text="Ok", font=('Arial', 26), command=lambda: ok_button_instant_player_choice())
    ok_button.pack(anchor='center', pady=5)
    
    go_back=tk.Button(instant_player_window, text="Go Back To Main Menu", font=("Arial", 25, "bold"), command=lambda: goback_main_menu_from_ips())
    go_back.pack(anchor='center', pady=6)
...
def instant_player_game():
    ...
    ip_gamewindow.title("SpecialChar Game!")
   
    
    header=tk.Label(ip_gamewindow, text="SpecialChar Game", font=("Impact", 80, "bold"))
    header.pack(fill="x", pady=5)
    
    rp_frame=tk.Frame(ip_gamewindow)
    ...
    chr_frame=tk.Frame(ip_gamewindow)
    chr_frame.pack(anchor="center", pady=20)
    chr_button_dict={}
    ...
    play_again=tk.Button(ip_gamewindow, text="Play Again", font=("Arial", 22, "bold"),
                         command=lambda: play_instant_player_again())
    ...
    back_to_ip_settings=tk.Button(ip_gamewindow, text="Back To Settings", font=("Arial", 22, "bold"),
                                  command=lambda: backto_ips_and_ipgame_destroy())
    back_to_ip_settings.pack(anchor='center', pady=10)
    
    status_bar=tk.Frame(ip_gamewindow, borderwidth=4, relief="raised", bg="red")
    status_bar.pack(side="bottom", fill="x", expand=1, anchor="s")
    
    status1=tk.Label(status_bar, text="Total attempts: "+str(attempts_choice), borderwidth=2, font=('Arial', 25), bg="red")
    status1.pack(side="left", fill="both", padx=5, expand=1)
    
    status2=tk.Label(status_bar, text="Attempts Remaining: "+str(attempts_choice-attempts_tillnow)+"/"+str(attempts_choice),
                     borderwidth=2, font=('Arial', 25), bg="red")
...
def game_intro_screen():
    ...
    charguess.title("SpecialChar Game")
    charguess.state("zoomed")    
    
  
    
    intro_label=tk.Label(charguess, text="SpecialChar Game!!", font=('Impact', 85))
    intro_label.pack( fill="x", pady=5)    
    
    Buttons_Frame=tk.Frame(charguess)
    Buttons_Frame.pack(pady=0, ipadx=10)
    
    Player_Choice=tk.Label(Buttons_Frame, text="Playing Modes", font=("Arial", 40, 'bold'))
    Player_Choice.pack(fill="x")
    
    Single_Player_Button=tk.Button(Buttons_Frame, text="Single Player", height=2, font=("Arial", 25, 'bold'),
                                   command=lambda: gotologin_and_chardestroy())
    Single_Player_Button.pack(padx=10, pady=4, fill="x")
    
    Double_Player_Button=tk.Button(Buttons_Frame, text="Double Player", height=2, font=("Arial", 25, 'bold'),
                                   command=lambda: dps_and_chardestroy())
    Double_Player_Button.pack(padx=10, pady=4, fill="x")
    
    Instant_Player_Button=tk.Button(Buttons_Frame, text="Instant Play", height=2, font=("Arial", 25, 'bold'),
